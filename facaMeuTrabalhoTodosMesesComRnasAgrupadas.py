...
def executa(i):
    ...
    cmd = ["python", "ExperimentoCenarios.py", str(i)]
    ...
    for nome in glob.glob("historicoDoTreino_ex_"+str(i)+'meses_*'):
        call(["mv", nome, 'experimentos/experimento_'+str(i)+'/'])
...
for i in range(numExecucoesPorExperimento):
    ...
    threads.append(th.Thread(target=executa, args=(i,)))
    threads[i].start()
